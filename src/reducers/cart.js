...
const cartReducer = (state=initState,action)=>{
	switch(action.type){
		case 'RECEIVE_STORE_ITEMS': return getStoreItems(state, action.data);
		case 'ADD_TO_CART': return addTocart(state, action.id);
		case 'HANDLE_ADD_Q': return addOne(state, action.id);
		case 'HANDLE_REMOVE_Q': return removeOne(state, action.id);
		case 'HANDLE_REMOVE': return removeItemFromCart(state, action.id);
		case 'CHANGE_TOTAL_PRICE':return changeTotalPrice(state, action.price);
		case 'UPDATE_CURRENT_CATEGORY':return updateCurrentCategory(state, action.data);
	}
	return state;
}
